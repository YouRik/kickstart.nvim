return {
  ...
  config = function(_, opts)
    ...
    vim.keymap.set('', '<leader><leader>s', function()
      hop.hint_char1()
    end, { remap = true, desc = 'Go to char in view' })
    vim.keymap.set('', '<leader><leader>f', function()
      hop.hint_char1 { direction = directions.AFTER_CURSOR, current_line_only = false }
    end, { remap = true, desc = 'f but down the whole view' })
    vim.keymap.set('', '<leader><leader>F', function()
      hop.hint_char1 { direction = directions.BEFORE_CURSOR, current_line_only = false }
    end, { remap = true, desc = 'F but up the whole view' })
    vim.keymap.set('', '<leader><leader>t', function()
      hop.hint_char1 { direction = directions.AFTER_CURSOR, current_line_only = false, hint_offset = -1 }
    end, { remap = true, desc = 't but down the whole view' })
    vim.keymap.set('', '<leader><leader>T', function()
